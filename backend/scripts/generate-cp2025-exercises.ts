...
async function generateExercisesFromCompetences(options: GenerateOptions) {
  ...
  const existingModule = await db
    .select()
    .from(modules)
    .where(eq(modules.titre, moduleTitle))
    .limit(1);
  ...
  if (existingModule.length === 0) {
    if (dryRun) {
      ...
      const [newModule] = await db.insert(modules).values({
        ...
        periode: periode ? `P${periode}` as any : 'P1' as any,
        ordre: 1,
    ...
    try {
      ...
      const variations = [
        ...
        },
        {
          ...
          ordre: 2,
          pointsReussite: template.pointsReussite + 5
        },
        {
          ...template,
          titre: `${template.titre} - Consolidation`,
          difficulte: 'consolidation' as const,
          ordre: 3,
      ...
      if (dryRun) {
        ...
        for (const variation of variations) {
          ...
          const [created] = await db.insert(exercises).values(exerciseData).returning();
