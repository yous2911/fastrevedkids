...
import React, { Suspense, ErrorBoundary, ComponentType } from 'react';
...
export const MascotSystemLazy = createLazyRoute({
  component: () => import('../../components/AdvancedMascotSystem'),
  priority: 'low',
  chunkName: 'mascot-3d',
  fallback: DetailedLoader,
});
...
export const preloadCriticalComponents = () => {
  lazyManager.preloadAssets(['dashboard', 'exercises'], {
    dashboard: { priority: 'high', preload: true },
    exercises: { priority: 'high', preload: true },
  });
};
...
export const preloadRoleBasedComponents = (userRole: 'student' | 'teacher' | 'admin') => {
  ...
  components.forEach(component => {
    lazyManager.preloadAssets([component], {
      [component]: { priority: 'medium', preload: true },
    });
  });
};
