...
export const useCP2025Backend = () => {
  const submitCompetenceResult = async (progressData: CP2025ProgressData) => {
    try {
      const response = await fetch('http://localhost:3003/api/competences/cp2025/progress', {
        method: 'POST',
  ...
  const getStudentCompetences = async (studentId: string) => {
    try {
      const response = await fetch(`http://localhost:3003/api/competences/cp2025/student/${studentId}`);
  ...
  const getRecommendedExercises = async (studentId: string) => {
    try {
      const response = await fetch(`http://localhost:3003/api/competences/cp2025/recommendations/${studentId}`);
      const data = await response.json();
