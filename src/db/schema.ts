...
// Enums
export const niveauScolaireEnum = ['CP', 'CE1', 'CE2', 'CM1', 'CM2'] as const;
export const matiereEnum = ['MATHEMATIQUES', 'FRANCAIS', 'SCIENCES', 'HISTOIRE_GEOGRAPHIE', 'ANGLAIS'] as const;
export const periodeEnum = ['P1', 'P2', 'P3', 'P4', 'P5'] as const;
export const typeExerciceEnum = ['QCM', 'CALCUL', 'TEXTE_LIBRE', 'DRAG_DROP', 'CONJUGAISON', 'LECTURE', 'GEOMETRIE', 'PROBLEME'] as const;
export const difficulteEnum = ['decouverte', 'consolidation', 'maitrise'] as const;
export const statutProgressionEnum = ['NON_COMMENCE', 'EN_COURS', 'TERMINE', 'MAITRISE'] as const;
...
export const students = mysqlTable('students', {
  ...
  age: int('age').notNull(),
  niveauActuel: mysqlEnum('niveau_actuel', niveauScolaireEnum).notNull(),
  emailParent: varchar('email_parent', { length: 100 }),
  ...
  adaptations: json('adaptations').$type<Record<string, any>>().default({}),
  ...
  emailIdx: index('email_idx').on(table.emailParent),
  niveauIdx: index('niveau_idx').on(table.niveauActuel),
  pointsIdx: index('points_idx').on(table.totalPoints),
}));

// Pedagogical modules table
export const modules = mysqlTable('modules', {
  id: serial('id').primaryKey(),
  titre: varchar('titre', { length: 200 }).notNull(),
  description: text('description').notNull(),
  niveau: mysqlEnum('niveau', niveauScolaireEnum).notNull(),
  matiere: mysqlEnum('matiere', matiereEnum).notNull(),
  periode: mysqlEnum('periode', periodeEnum).notNull(),
...
// Pedagogical exercises table
export const exercises = mysqlTable('exercises', {
  id: serial('id').primaryKey(),
  titre: varchar('titre', { length: 200 }).notNull(),
  consigne: text('consigne').notNull(),
  type: mysqlEnum('type', typeExerciceEnum).notNull(),
  difficulte: mysqlEnum('difficulte', difficulteEnum).default('decouverte').notNull(),
  pointsReussite: int('points_reussite').default(10).notNull(),
  dureeEstimee: int('duree_estimee').default(5).notNull(), // in minutes
  ordre: int('ordre').default(1).notNull(),
  moduleId: int('module_id').notNull(),
  configuration: json('configuration').$type<Record<string, any>>().notNull(),
  actif: boolean('actif').default(true).notNull(),
  metadata: json('metadata').$type<Record<string, any>>().default({}),
  createdAt: timestamp('created_at').defaultNow().notNull(),
  updatedAt: timestamp('updated_at').defaultNow().onUpdateNow().notNull(),
...
export const progress = mysqlTable('progress', {
  ...
  historique: json('historique').$type<Array<any>>().default([]),
  createdAt: timestamp('created_at').defaultNow().notNull(),
  updatedAt: timestamp('updated_at').defaultNow().onUpdateNow().notNull(),
}, (table) => ({
...
export const sessions = mysqlTable('sessions', {
  ...
  pointsGagnes: int('points_gagnes').default(0).notNull(),
  actionsUtilisateur: json('actions_utilisateur').$type<Array<any>>().default([]),
  metadata: json('metadata').$type<Record<string, any>>().default({}),
  createdAt: timestamp('created_at').defaultNow().notNull(),
  updatedAt: timestamp('updated_at').defaultNow().onUpdateNow().notNull(),
}, (table) => ({
